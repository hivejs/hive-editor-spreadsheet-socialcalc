...
exports.apply = function(scSnapshot, ops) {
  ...
  socialCalcControl.sheet.ParseSheetSave(scSnapshot)
  ...
  .filter((op) => !!op)
  .forEach((cmd) => {
    var error = SocialCalc.ExecuteSheetCommand(scInstance.sheet, cmd, /*saveundo:*/false)
...
exports.deserializeEdit = function(cmds) {
  ...
  .reduce((ops, cmd) => {
    ...
    if(op) ops.push(op) // If nothing recognizes this, we filter it out.
    return ops
  }, [])
}

exports.serializeEdit = function(ops) {
  return ops.map((op) => op.serialize()).join('\n')
}
...
Set.prototype.hydrate = function(obj) {
...
Set.parse = function(cmd) {
  if(0 !== cmd.indexOf('set')) return
  var parts = cmd.split(' ')
    , cmd = parts[0]
    , target = parts[1]
    , attr = parts[2]
    , value = cmd.substr(cmd.length+1+target.length+1+attr.length+1)
